...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  var validateFileURL = function (file) {
    ...
    // Dogear: allow loading any http(s) PDF into the bundled viewer; the
    // extension has <all_urls> host permission, so the fetch is authorized.
    // file: is included so locally opened PDFs work when the user has granted
    // "Allow access to file URLs". Other schemes stay blocked.
    if (window.location.protocol === "chrome-extension:") {
      const fileProtocol = URL.parse(file, window.location)?.protocol;
      if (fileProtocol === "http:" || fileProtocol === "https:" || fileProtocol === "file:") {
        return;
      }
      const ex = new Error("unsupported file scheme");
